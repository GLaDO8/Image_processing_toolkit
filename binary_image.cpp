...
binary_image::binary_image(Image i, int treshold): Image(i) {binarize(treshold);}

void binary_image::binarize(int treshold) {
    for(int i = 0; i < _h; i++) {
        for(int j = 0; j < _w; j++) {
            if(_arr[i][j].get_brightness() > treshold)   _arr[i][j].set_color(255, 255, 255);
            else _arr[i][j].set_color(0, 0, 0);
        }
    }
}
